...
const EditList = ({ editToDo }) => {
  ...
  return (
    ...
    <>
    {editList && (
        <div className="w-[67.5%] -me-12 bg-[var(--secondary-color)] ms-2 ps-6 pt-8 relative">
            ...
            <p className="py-4 px-6 cursor-pointer text-sm text-[var(--secondary-text)]">Add Notes</p>
            <div className={`flex flex-col fixed bottom-8 transition-all duration-100 ${menu ? "w-84" : "w-112"}`}>
                <hr className="border-[#D8e0e9]"/>
                <div className="flex justify-between mt-8 px-4">
                  <button className="flex items-center gap-2 text-sm text-[var(--primary-text)] cursor-pointer" onClick={() => setEditList(false)}><MdOutlineClose size={23} /></button>
                  <p className="text-[var(--secondary-text)]">Created Today</p>
                  <button className="flex items-center gap-2 text-sm text-[var(--primary-text)] cursor-pointer"><HiTrash size={22} /></button>
                </div>
            </div>
        </div>
    )}
